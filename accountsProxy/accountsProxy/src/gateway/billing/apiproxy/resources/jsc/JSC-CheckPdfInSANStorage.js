...
var routeRequest = "local";
	for (var i = 0; i < dirArray.length; i++) {
		// exchObjArray.push(httpClient.get("/files?dir_name="+dirArray[i]+"&file_name="+documentRefNumber+".pdf&operation=exists"));
		var url = sanStorageUrl+"?dir_name="+dirArray[i]+"&file_name="+documentRefNumber+".pdf&operation=exists";
		var operation = "GET";
	...
	for (var i = 0; i < exchObjArray.length; i++) {
		...
		if (fileAvailableInDir) {
			...
			context.setVariable("searchFileName",documentRefNumber+".pdf");
		    ...
		    context.setVariable('api-error.code', 'NB_40002');
				...
				context.setVariable('api-error.message', 'PDF File not found in given directories');
